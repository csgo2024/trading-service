...
public class GlobalState
{
    ...
    public override string ToString()
    {
        var snapshot = new
        {
            Alerts = _alerts.Values().Select(x => $"{x.Symbol}-{x.Interval}"),
            Strategies = _strategies.Values().Select(x => $"{x.Symbol}-{x.Interval}-{x.AccountType}-{x.StrategyType}"),
            Symbols = string.Join(", ", _stream.GetAllSymbols()),
            Intervals = string.Join(", ", _stream.GetAllIntervals()),
            Tasks = _taskState.Values().Select(x => $"{x.Category.ToString().ToLower()}-{x.Id}"),
